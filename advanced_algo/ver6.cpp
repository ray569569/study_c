...
// TargetedPathFinder: Optimized for t2.in
class TargetedPathFinder {
...
public:
    ...
    double getCost(int x, int y, int netId, bool strict = false) {
        ...
        // Guide Net 1 and Net 5 to x=15-18
            ...
            if (x == net.ex && y == net.ey) {
                ...
                if (strict) {
                    for (auto [px, py] : net.path) {
                        if (usage[py][px] > 1) { // Allow single overlap
                            net.path.clear();
                            return false;
                        }
                    }
    ...
    double evaluateRerouteScore(const Net& net, int iteration) {
        ...
        double lengthWeight = 0.01; // Further reduced
    ...
    bool negotiateConflicts(int newNetIdx, int iteration) {
        ...
        // Randomly select among top candidates
        random_device rd;
        mt19937 g(rd());
        sort(conflictNets.begin(), conflictNets.end(), 
             [](const auto& a, const auto& b) { return a.second > b.second; });
        int topN = min(3, (int)conflictNets.size());
        shuffle(conflictNets.begin(), conflictNets.begin() + topN, g);
    ...
    void solve() {
        ...
        cout << "=== Targeted PathFinder ===" << endl;
        cout << "Nets: " << nets.size() << ", Grid: " << W << "x" << H << endl;
        
        // Prioritize Net 2 (id=1)
        ...
        sort(netOrder.begin(), netOrder.end(), [&](const auto& a, const auto& b) {
            if (a.first == 1) return true; // Net 2 first
        ...
        while (iteration < MAX_ITER) {
            ...
            if (iteration % 10 == 0) {
                historyIncrement *= 1.2; // Slower increase
            }
            ...
            if (routed == lastRouted && newlyRouted == 0) {
                stagnantCount++;
                if (stagnantCount >= 3) { // Further reduced
            ...
            if (elapsed > 900) {
                cout << "Time limit approaching..." << endl;
                break;
            }
