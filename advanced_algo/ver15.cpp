...
public:
    ...
    void solve() {
        ...
        while (iteration < MAX_ITER) {
            ...
            if (nets.size() >= 30 && iteration % 10 == 0) {
                historyIncrement *= 1.3;  // 更快增加歷史成本
...
struct SolutionScore {
    ...
    int routed;           // 成功繞線數（最重要）
    int totalLength;      // 總線長（次要）
    int longestLength;    // 最長單一線長（第三）
    int totalBends;       // 總彎折數（第四）
    
    // 比較函數：按照評分優先順序
    bool operator<(const SolutionScore& other) const {
        // 1. 先比繞線數（越多越好）
        if (routed != other.routed) {
            return routed > other.routed;
        }
        // 2. 繞線數相同，比總線長（越短越好）
        ...
        if (longestLength != other.longestLength) {
            return longestLength < other.longestLength;
        }
        // 4. 最後比彎折數（越少越好）
...
int main(int argc, char* argv[]) {
    ...
    const int NUM_RUNS = 100;  // 減少到50次，通常足夠了
